...
const SeatLayout = () => {
  ...
  const getShow = async () => {
    const show = dummyShowsData.find(show => show._id.toString() === id.toString())
    if (show){
      setShow({
        movie: show,
        dateTime: dummyDateTimeData
      ...
      {Array.from({ length: count }, (_, i) => {
        ...
        return (
          <button
            ...
            className={`h-8 w-8 rounded border border-primary/60 cursor-pointer text-sm ${selectedSeats.includes(seatId) ? "bg-primary text-white" : ""}`}
          >
            {seatId}
          </button>
        )
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-4 md:px-10 lg:px-20 py-16'>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-10 overflow-x-auto no-scrollbar'>
        ...
        <button onClick={() => navigate('/my-bookings')} className='flex item-center gap-1 mt-20 px-10 py-3 text-s, bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95'>
          Proceed To Checkout
